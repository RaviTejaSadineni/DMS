...
| # | Feature | Description |
|---|---------|-------------|
| 1 | **Gemini PDF Extraction** | Replace regex parser with Gemini Gemini vision — upload PDF as image, AI reads and extracts all fields regardless of layout (tables, sideways text, scattered data). Configure via `GEMINI_API_KEY` and `GEMINI_API_KEY` env vars. |
...
| 9 | **Handwritten text OCR** | Handle scanned/handwritten delivery orders where text isn't digitally embedded — use Gemini Vision. |
...
| 12 | **PDF attachment storage** | Store the original PDF in the database (or Cloud Storage) linked to the booking — viewable anytime from entries grid. |
...
# Gemini (for AI PDF extraction)
GEMINI_API_KEY=https://your-resource.openai.aistudio.google.com
...
# Optional: Cloud Storage (for PDF storage)
