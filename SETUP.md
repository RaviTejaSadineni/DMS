...
# ---------- GEMINI (OPTIONAL — for AI PDF extraction) ----------
...
# Feature #1, it will use Gemini Gemini vision for intelligent extraction.
#
# To get these values:
#   1. Go to https://portal.aistudio.google.com
#   2. Search "Gemini" → Create a resource (or use existing)
#   3. Go to your resource → "Keys and Endpoint" in left menu
#   4. Copy "Endpoint" → paste as GEMINI_API_KEY
#   5. Copy "Key 1" → paste as GEMINI_API_KEY
#   6. Go to "Model deployments" → "Manage Deployments"
#   7. Deploy a model (recommended: gpt-4o)
#   8. The deployment name you chose → paste as GEMINI_MODEL
#
...
# ---------- CLOUD STORAGE (OPTIONAL — for storing uploaded PDFs) ----------
...
All values available via os.getenv("KEY_NAME") anywhere in backend code:
    ...
    - main.py reads GEMINI_API_KEY and GEMINI_MODEL
```
...
| `GEMINI_API_KEY` | `backend/main.py` | line 24 | Gemini API key |
| `GEMINI_API_KEY` | `backend/main.py` | (future) | Gemini API key |
| `
